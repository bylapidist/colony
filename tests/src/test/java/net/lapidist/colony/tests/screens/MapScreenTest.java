...
public class MapScreenTest {
    ...
    private static void dispatchPause() {
        net.lapidist.colony.events.Events.dispatch(new net.lapidist.colony.client.events.PauseEvent());
    }

    private static void dispatchResume() {
        net.lapidist.colony.events.Events.dispatch(new net.lapidist.colony.client.events.ResumeEvent());
    }

    private static void dispatchSpeed(final double value) {
        net.lapidist.colony.events.Events.dispatch(new net.lapidist.colony.client.events.SpeedMultiplierEvent(value));
    }
    ...
    public void delegatesLifecycleToHandlerAndProcessesWorld() throws Exception {
        ...
        try (MockedConstruction<SpriteBatch> ignored = mockConstruction(SpriteBatch.class);
            ...
            uiStatic.when(() -> MapUiBuilder.build(
                    ...
                    eq(client),
                    eq(colony),
                    any(MapScreenEventHandler.class)))
    ...
    public void viewportUsesUiScale() throws Exception {
        ...
        try (MockedConstruction<SpriteBatch> ignored = mockConstruction(SpriteBatch.class);
            ...
            uiStatic.when(() -> MapUiBuilder.build(
                    any(Stage.class), any(World.class), eq(client), eq(colony), any(MapScreenEventHandler.class)))
                    .thenAnswer(inv -> new MapUi(
                            inv.getArgument(0),
                            mock(MinimapActor.class),
                            mock(net.lapidist.colony.client.ui.ChatBox.class)));
    ...
    @Test
    public void pausedSkipsProcessing() throws Exception {
        Colony colony = mock(Colony.class);
        Settings settings = new Settings();
        when(colony.getSettings()).thenReturn(settings);
        MapState state = new MapState();
        ...
        try (MockedConstruction<SpriteBatch> ignored = mockConstruction(SpriteBatch.class);
            ...
            worldStatic.when(() -> MapWorldBuilder.build(any(), isNull(), eq(settings), any(),
                    ...
                    .thenReturn(world);
            uiStatic.when(() -> MapUiBuilder.build(
                    any(Stage.class), eq(world), eq(client), eq(colony), any(MapScreenEventHandler.class)))
                    .thenAnswer(inv -> new MapUi(
                            inv.getArgument(0),
                            mock(MinimapActor.class),
                            mock(net.lapidist.colony.client.ui.ChatBox.class)));

            net.mostlyoriginal.api.event.common.EventSystem system =
                    new net.mostlyoriginal.api.event.common.EventSystem();
            net.lapidist.colony.events.Events.init(system);
    ...
    @Test
    public void slowMotionMultipliesDelta() throws Exception {
        Colony colony = mock(Colony.class);
        Settings settings = new Settings();
        when(colony.getSettings()).thenReturn(settings);
        MapState state = new MapState();
