...
`graphics.softShadows` enables smoother edges around dynamic light shadows. The
blur amount is configured by `graphics.shadowSoftnessLength` which defaults to
`2`.
