...
public final class KeyboardInputHandler {
    ...
    public KeyboardInputHandler(final PlayerCameraSystem cameraSystemToSet, final Settings settingsToUse) {
    ...
    public void handleKeyboardInput(final float deltaTime) {
        ...
        if (Gdx.input.isKeyPressed(settings.getUpKey())) {
            position.y += moveAmount;
        }
        if (Gdx.input.isKeyPressed(settings.getDownKey())) {
            position.y -= moveAmount;
        }
        if (Gdx.input.isKeyPressed(settings.getLeftKey())) {
            position.x -= moveAmount;
        }
        if (Gdx.input.isKeyPressed(settings.getRightKey())) {
            position.x += moveAmount;
        }
    }
